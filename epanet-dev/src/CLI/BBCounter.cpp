...
bool BBCounter::update_y(bool is_feasible) {
  if (this->h < 0 || this->h > this->h_max) {
    throw std::out_of_range("ERR: h (" + std::to_string(this->h) +
                            ") is out of range in update_y");
  }
...
void BBCounter::jump_to_end() { this->y[this->h] = this->y_max; }
...
void BBCounter::calc_actuations_csum(int *actuations_csum,
                                     const std::vector<int> &x, int h) {
