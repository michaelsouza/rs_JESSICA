...
bool test_cost() {
  ...
  for (int h = 1; h <= h_max; ++h) {
    ...
    if (!updated) {
      ColorStream::println("Error: Failed to update x for hour " +
                               std::to_string(h) + ".",
                           ColorStream::Color::RED);
      return false;
    }
  ...
  bool is_feasible =
      process_node(inpFile, counter, stats, nodes, tanks, pump_names, verbose);
  if (!is_feasible) {
    ColorStream::println("Error: Process node returned infeasible solution.",
                         ColorStream::Color::RED);
    return false;
  }
  ...
  if (std::abs(stats.cost_min - expected_cost_min) > tolerance) {
    ColorStream::println(
        "Test Failed: stats.cost_min (" + std::to_string(stats.cost_min) +
  ...
  // If the check passes
  ColorStream::println("Test Passed: stats.cost_min (" +
                           std::to_string(stats.cost_min) + ") is within " +
                           std::to_string(tolerance) + " of expected " +
                           std::to_string(expected_cost_min) + ".",
...
void test_all() {
  if (!test_cost()) {
    ...
    ColorStream::println("All tests passed successfully!",
                         ColorStream::Color::GREEN);
  }
}
