...
void update_pumps_pattern_speed(std::vector<Pump *> pumps, BBCounter &counter, bool verbose) {
...
bool check_pressures(EN_Project p, std::map<std::string, int> &nodes, bool verbose) {
    if (verbose) {
        std::cout << "\nChecking pressures: [";
...
bool check_levels(EN_Project p, std::map<std::string, int> &tanks, bool verbose) {
    if (verbose) {
        std::cout << "\nChecking levels: [";
...
void show_stability(bool is_feasible, const std::string &tank_name, double level,
...
bool check_stability(EN_Project p, std::map<std::string, int> &tanks, bool verbose) {
    if (verbose) {
        std::cout << "\nChecking stability: [";
        for (const auto &tank : tanks)
            std::cout << tank.first << " ";
...
void get_nodes_and_tanks_ids(const char *inpFile, std::map<std::string, int> &nodes,
...
void show_nodes_pumps_tanks(const std::map<std::string, int> &nodes,
                            ...
                            const std::map<std::string, int> &tanks, bool verbose) {
    if (verbose) {
        std::cout << "\nNodes: [ ";
        for (const auto &node : nodes)
            std::cout << node.first << " ";
...
bool process_node(const char *inpFile, BBCounter &counter, BBStats &stats,
                 const std::map<std::string, int> &nodes, const std::map<std::string, int> &tanks,
    ...
    do {
        ...
        is_feasible = check_pressures(p, const_cast<std::map<std::string, int>&>(nodes), verbose);
        if (!is_feasible) {
        ...
        // Check tank levels
        is_feasible = check_levels(p, const_cast<std::map<std::string, int>&>(tanks), verbose);
        if (!is_feasible) {
    ...
    if (is_feasible && counter.h == counter.h_max) {
        is_feasible = check_stability(p, const_cast<std::map<std::string, int>&>(tanks), verbose);
        if (!is_feasible) {
