...
int DataManager::getCount(int element, int *count, Network *nw) {
  ...
  *count = 0;
  switch (element) {
  case EN_NODECOUNT:
    *count = nw->count(Element::NODE);
    break;
  case EN_LINKCOUNT:
    *count = nw->count(Element::LINK);
    break;
  case EN_PATCOUNT:
    *count = nw->count(Element::PATTERN);
    break;
  case EN_CURVECOUNT:
    *count = nw->count(Element::CURVE);
    break;
  case EN_CONTROLCOUNT:
    *count = nw->count(Element::CONTROL);
    break;
  case EN_RULECOUNT:
    break;
  case EN_TANKCOUNT:
...
int DataManager::getNodeValue(int index, int param, double *value,
                              Network *nw) {
  *value = 0.0;
  if (index < 0 || index >= nw->count(Element::NODE))
    return 205;
  ...
  default:
    return getTankValue(param, node, value, nw);
  }
  return 0;
}
...
int DataManager::getLinkNodes(int index, int *fromNode, int *toNode,
                              Network *nw) {
  *fromNode = -1;
  *toNode = -1;
  if (index < 0 || index >= nw->count(Element::LINK))
...
int DataManager::getLinkValue(int index, int param, double *value,
                              Network *nw) {
  *value = 0.0;
  if (index < 0 || index >= nw->count(Element::LINK))
    return 205;
  ...
  case EN_LEAKAGE:
    *value = link->leakage * nw->ucf(Units::FLOW);
    break;
  default:
    return getPipeValue(param, link, value, nw);
  }
  return 0;
}
  ...
  *value = 0.0;
  if (node->qualSource) {
    ...
    case EN_SOURCEPAT:
      ...
      break;
    case EN_SOURCETYPE:
      *value = node->qualSource->type;
      break;
    case EN_SOURCEMASS:
      if (node->qualSource->type == QualSource::MASS)
        *value = node->qualSource->strength / 60.0;
      else
        *value = node->qualSource->strength * FT3perL;
      break;
    }
  } else if (param == EN_SOURCEPAT)
    *value = -1;
  return 0;
}
