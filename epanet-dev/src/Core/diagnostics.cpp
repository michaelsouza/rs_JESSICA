...
void Diagnostics::validateNetwork(Network *nw) {
  ...
  if (!hasValidLinks(nw))
    result = false;
  if (!hasValidValves(nw))
    result = false;
  if (!hasValidCurves(nw))
    result = false;
  if (!hasConnectedNodes(nw))
    result = false;
  if (result == false) {
  ...
  for (Link *link : nw->links) {
    ...
    try {
      ...
      if ((link->toNode->fixedGrade && valve->valveType == Valve::PRV) ||
        ...
        throw NetworkError(NetworkError::ILLEGAL_VALVE_CONNECTION, valve->name);
      }
    } catch (ENerror const &e) {
      nw->msgLog << e.msg;
      result = false;
  ...
  for (int i = 0; i < nodeCount; i++) {
    try {
      if (!marked[i]) {
        unmarkedCount++;
        if (unmarkedCount <= 10)
          throw NetworkError(NetworkError::UNCONNECTED_NODE, nw->node(i)->name);
      }
    } catch (ENerror const &e) {
      nw->msgLog << e.msg;
    }
