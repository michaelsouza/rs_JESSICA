...
int EN_runEpanet(const char *inpFile, const char *rptFile,
                 const char *outFile) {
  std::cout << "\n... EPANET Version 3.0\n";
  ...
  for (;;) {
    ...
    if ((err = p.openReport(rptFile)))
      break;
    std::cout << "\n    Reading input file ...";
    if ((err = p.load(inpFile)))
      break;
    if ((err = p.openOutput(outFile)))
      break;
    p.writeSummary();

    // ... initialize the solver
    std::cout << "\n    Initializing solver ...";
    if ((err = p.initSolver(false)))
      break;
    std::cout << "\n    ";
    ...
    do {
      ...
      // ... advance solver to next period in time while solving for water
      // quality
      if (!err)
        err = p.advanceSolver(&tstep);
    } while (tstep > 0 && !err);
    break;
  }
...
int EN_runSolver(int *t, EN_Project p) { return project(p)->runSolver(t); }
...
int EN_saveOutput(EN_Project p) { return project(p)->saveOutput(); }
...
int EN_writeReport(EN_Project p) { return project(p)->writeReport(); }
...
int EN_getNodeValue(int index, int param, double *value, EN_Project p) {
  return DataManager::getNodeValue(index, param, value,
...
int EN_getLinkValue(int index, int param, double *value, EN_Project p) {
  return DataManager::getLinkValue(index, param, value,
                                   project(p)->getNetwork());
}
