...
Node *Network::node(const int index) { return nodes[index]; }
...
Link *Network::link(const int index) { return links[index]; }
...
Pattern *Network::pattern(const int index) { return patterns[index]; }
...
Curve *Network::curve(const int index) { return curves[index]; }
...
Control *Network::control(const int index) { return controls[index]; }
...
void Network::convertUnits() {
  ...
  for (Node *node : nodes)
    node->convertUnits(this);
  for (Link *link : links)
    link->convertUnits(this);
  for (Control *control : controls)
    control->convertUnits(this);
}
...
bool Network::createHeadLossModel() {
  if (headLossModel)
    delete headLossModel;
  headLossModel = HeadLossModel::factory(option(Options::HEADLOSS_MODEL),
                                         option(Options::KIN_VISCOSITY));
...
bool Network::createDemandModel() {
  if (demandModel)
    delete demandModel;
  demandModel = DemandModel::factory(option(Options::DEMAND_MODEL),
                                     option(Options::PRESSURE_EXPONENT));
...
bool Network::createLeakageModel() {
  ...
  leakageModel = LeakageModel::factory(option(Options::LEAKAGE_MODEL),
                                       ucf(Units::LENGTH), ucf(Units::FLOW));
