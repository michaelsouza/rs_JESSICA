...
static const char *demandModelWords[] = {"FIXED", "CONSTRAINED", "POWER",
                                         "LOGISTIC", 0};

// Leakage model keywords
static const char *leakageModelWords[] = {"NONE", "POWER", "FAVAD", 0};
...
int Options::setOption(IndexOption option, const string &value,
                       Network *network) {
  int i;
  ...
  case TRACE_NODE:
    ...
    if (i < 0)
      return InputError::UNDEFINED_OBJECT;
...
void Options::setOption(ValueOption option, double value) {
  if (option == KIN_VISCOSITY && value > 1.0e-3)
    value *= VISCOSITY;
  if (option == MOLEC_DIFFUSIVITY && value > 1.0e-3)
    value *= DIFFUSIVITY;
...
void Options::setReportFieldOption(int type, int index, int enabled,
                                   int precision, double lowerLimit,
                                   double upperLimit) {
  reportFields.setField(type, index, enabled, precision, lowerLimit,
                        upperLimit);
}
...
void Options::adjustOptions() {
  ...
  if (timeOptions[TOTAL_DURATION] == 0)
    indexOptions[QUAL_TYPE] = NOQUAL;
  ...
  if (indexOptions[REPORT_TRIALS] == true)
    indexOptions[REPORT_STATUS] = true;
}
...
string Options::reactOptionsToStr() {
  ...
  s << setw(w) << "LIMITING POTENTIAL" << valueOptions[LIMITING_CONCEN] << "\n";
  s << setw(w) << "ROUGHNESS CORRELATION" << valueOptions[ROUGHNESS_FACTOR]
    << "\n";
  return s.str();
}
...
string Options::timeOptionsToStr() {
  ...
  s << setw(w) << "RULE TIMESTEP" << Utilities::getTime(timeOptions[RULE_STEP])
    << "\n";
