...
QualEngine::QualEngine()
    : engineState(QualEngine::CLOSED), network(nullptr), qualSolver(nullptr),
      nodeCount(0), linkCount(0), qualTime(0), qualStep(0) {}
...
void QualEngine::open(Network *nw) {
  ...
  if (engineState != QualEngine::CLOSED)
    close();
...
void QualEngine::init() {
  if (engineState != QualEngine::OPENED)
    return;

  // ... initialize node concentrations & tank volumes

  for (Node *node : network->nodes) {
    if (network->qualModel->type == QualModel::TRACE)
      node->quality = 0.0;
    else
      node->quality = node->initQual;
...
void QualEngine::solve(int tstep) {
  ...
  if (engineState != QualEngine::INITIALIZED)
    return;
  if (tstep == 0)
    return;
...
void QualEngine::setSourceQuality() {
  ...
  for (Link *link : network->links) {
    ...
    if (fromNode->qualSource)
      fromNode->qualSource->outflow += abs(q);
  }
}
