...
Control::Control(int type_, string name_)
    : Element(name_), type(type_), link(nullptr), status(NO_STATUS),
      setting(0.0), node(nullptr), head(0.0), volume(0.0), levelType(NO_LEVEL),
      time(0) {}
...
void Control::setProperties(int controlType, Link *controlLink, int linkStatus,
                            double linkSetting, Node *controlNode,
                            double nodeSetting, int controlLevelType,
                            int timeSetting) {
  type = controlType;
  link = controlLink;
  if (linkStatus != NO_STATUS)
    status = linkStatus;
  else
    setting = linkSetting;
  node = controlNode;
...
void Control::convertUnits(Network *network) {
  ...
  if (link)
    setting = link->convertSetting(network, setting);
}
...
int Control::timeToActivate(Network *network, int t, int tod) {
  ...
  case TIME_OF_DAY:
    if (time >= tod)
      ...
      aTime = 86400 - tod + time;
    break;
  }

  if (aTime > 0 && activate(makeChange, network->msgLog))
    return aTime;
  else
    return -1;
}
...
void Control::apply(Network *network, int t, int tod) {
  ...
  case TANK_LEVEL:
    ...
    if ((levelType == LOW_LEVEL &&
         tank->volume <= volume + abs(tank->outflow)) ||
        (levelType == HI_LEVEL &&
         tank->volume >= volume - abs(tank->outflow))) {
...
bool Control::activate(bool makeChange, ostream &msgLog) {
  ...
  if (status != NO_STATUS) {
    ...
    reason =
        linkStr + s_SettingChanged + Utilities::to_string(setting) + reason;
