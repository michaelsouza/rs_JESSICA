...
Junction::Junction(string name_)
    : Node(name_), pMin(MISSING), pFull(MISSING), emitter(nullptr) {}
...
void Junction::convertUnits(Network *nw) {
  ...
  if (demands.size() == 0)
    demands.push_back(primaryDemand);
  ...
  if (emitter)
    emitter->convertUnits(nw);

  // ... use global pressure limits if no local limits assigned

  if (pMin == MISSING)
    pMin = nw->option(Options::MINIMUM_PRESSURE);
  if (pFull == MISSING)
    pFull = nw->option(Options::SERVICE_PRESSURE);
...
double Junction::findEmitterFlow(double h, double &dqdh) {
  dqdh = 0.0;
  if (emitter)
    return emitter->findFlowRate(h - elev, dqdh);
  return 0;
}
