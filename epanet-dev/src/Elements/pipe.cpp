...
Pipe::Pipe(string name)
    : Link(name), hasCheckValve(false), length(0.0), roughness(0.0),
      resistance(0.0), lossFactor(0.0), leakCoeff1(MISSING),
      leakCoeff2(MISSING), bulkCoeff(MISSING), wallCoeff(MISSING),
      massTransCoeff(0.0) {}
...
void Pipe::convertUnits(Network *nw) {
  ...
  if (leakCoeff1 == MISSING)
    leakCoeff1 = nw->option(Options::LEAKAGE_COEFF1);
  if (leakCoeff2 == MISSING)
    leakCoeff2 = nw->option(Options::LEAKAGE_COEFF2);

  // ... apply global default reaction coeffs.

  if (bulkCoeff == MISSING)
    bulkCoeff = nw->option(Options::BULK_COEFF);
  if (wallCoeff == MISSING)
    wallCoeff = nw->option(Options::WALL_COEFF);
}
...
void Pipe::setInitStatus(int s) { initStatus = s; }

//-----------------------------------------------------------------------------

void Pipe::setInitSetting(double s) {
  if (s == 0.0)
    initStatus = LINK_CLOSED;
  else
...
double Pipe::getUnitHeadLoss() {
  if (length > 0.0)
    return abs(hLoss) * 1000.0 / length;
  return 0.0;
}
...
void Pipe::findHeadLoss(Network *nw, double q) {
  if (status == LINK_CLOSED || status == TEMP_CLOSED) {
    ...
    if (hasCheckValve)
      HeadLossModel::addCVHeadLoss(q, hLoss, hGrad);
  }
}
...
bool Pipe::changeStatus(int s, bool makeChange, const string reason,
                        ostream &msgLog) {
  if (status != s) {
    if (makeChange) {
      msgLog << "\n    " << reason;
