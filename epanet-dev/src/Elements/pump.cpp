...
Pump::Pump(string name_)
    : Link(name_), speed(1.0), speedPattern(nullptr), efficCurve(nullptr),
      costPattern(nullptr), costPerKwh(0.0) {}
...
void Pump::validate(Network *nw) {
  if (pumpCurve.curve || pumpCurve.horsepower > 0.0) {
    ...
    if (err)
      throw NetworkError(err, name);
  } else
    throw NetworkError(NetworkError::NO_PUMP_CURVE, name);
}
...
bool Pump::changeStatus(int s, bool makeChange, const string reason,
                        ostream &msgLog) {
  if (status != s) {
    if (makeChange) {
      if (s == LINK_OPEN && speed == 0.0)
        speed = 1.0;
      if (s == LINK_CLOSED) {
        flow = ZERO_FLOW;
        speed = 0.0;
...
bool Pump::changeSetting(double s, bool makeChange, const string reason,
                         ostream &msgLog) {
  if (speed != s) {
