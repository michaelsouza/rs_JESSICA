...
PumpCurve::PumpCurve()
    : curveType(NO_CURVE), curve(nullptr), horsepower(0.0), qInit(0.0),
      qMax(0.0), hMax(0.0), h0(0.0), r(0.0), n(0.0), qUcf(1.0), hUcf(1.0) {}
...
void PumpCurve::setCurve(Curve *c) { curve = c; }
...
int PumpCurve::setupCurve(Network *network) {
  ...
  // ... a pump curve was supplied
    ...
    // ... custom curve supplied
      ...
      err = setupCustomCurve();
  }

  // ... error - no curve was supplied

  else
    err = NetworkError::NO_PUMP_CURVE;
  qInit /= qUcf;
  return err;
}
...
void PumpCurve::findHeadLoss(double speed, double flow, double &headLoss,
                             double &gradient) {
  double q = abs(flow);
  switch (curveType) {
  case CUSTOM:
    customCurveHeadLoss(speed, q, headLoss, gradient);
    break;

  case CONST_HP:
    constHpHeadLoss(speed, q, headLoss, gradient);
    break;

  case POWER_FUNC:
    powerFuncHeadLoss(speed, q, headLoss, gradient);
    break;
  }
}
...
int PumpCurve::setupPowerFuncCurve() {
  ...
  if (h0 < TINY_NUMBER || h0 - h1 < TINY_NUMBER || h1 - h2 < TINY_NUMBER ||
      q1 < TINY_NUMBER || q2 - q1 < TINY_NUMBER)
    return NetworkError::INVALID_PUMP_CURVE;
...
int PumpCurve::setupCustomCurve() {
  ...
  double slope = (curve->y(0) - curve->y(1)) / (curve->x(1) - curve->x(0));
  hMax = curve->y(0) + slope * curve->x(0);

  // ... extrapolate to zero head to find max. flow

  int k = curve->size() - 1;
  slope = (curve->x(k) - curve->x(k - 1)) / (curve->y(k - 1) - curve->y(k));
...
void PumpCurve::customCurveHeadLoss(double speed, double flow, double &headLoss,
                                    double &gradient) {
...
void PumpCurve::constHpHeadLoss(double speed, double flow, double &headLoss,
                                double &gradient) {
...
void PumpCurve::powerFuncHeadLoss(double speed, double flow, double &headLoss,
                                  double &gradient) {
