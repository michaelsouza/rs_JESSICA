...
const char *QualSource::SourceTypeWords[] = {"CONCEN", "MASS", "FLOWPACED",
                                             "SETPOINT", 0};

//-----------------------------------------------------------------------------

QualSource::QualSource()
    : type(CONCEN), base(0.0), pattern(nullptr), strength(0.0), outflow(0.0),
      quality(0.0) {}
...
void QualSource::setStrength(Node *node) {
  strength = base;
  if (pattern)
    strength *= pattern->currentFactor();
  if (type == MASS)
    strength *= 60.0; // mass/min -> mass/sec
  else
    strength /= FT3perL; // mass/L -> mass/ft3
}
