...
const char *Valve::ValveTypeWords[] = {"PRV", "PSV", "FCV",
                                       "TCV", "PBV", "GPV"};
...
Valve::Valve(string name_)
    : Link(name_), valveType(TCV), lossFactor(0.0), hasFixedStatus(false),
      elev(0.0) {
...
string Valve::typeStr() { return ValveTypeWords[valveType]; }
...
double Valve::getSetting(Network *nw) {
  ...
  case PSV:
  case PBV:
    return setting * nw->ucf(Units::PRESSURE);
  case FCV:
    return setting * nw->ucf(Units::FLOW);
  default:
    return setting;
  }
}
...
void Valve::updateStatus(double q, double h1, double h2) {
  ...
  switch (valveType) {
  case PRV:
    newStatus = updatePrvStatus(q, h1, h2);
    break;
  case PSV:
    newStatus = updatePsvStatus(q, h1, h2);
    break;
  default:
    break;
  }
...
bool Valve::changeSetting(double newSetting, bool makeChange,
                          const string reason, ostream &msgLog) {
...
bool Valve::changeStatus(int newStatus, bool makeChange, const string reason,
                         ostream &msgLog) {
