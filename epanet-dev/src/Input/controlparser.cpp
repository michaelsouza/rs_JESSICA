...
ControlParser::ControlParser() { initializeControlSettings(); }
...

{
  ...
  if (Utilities::match(keyword, w_NODE))
    parseLevelSetting(network);

  //... get control time if keyword == TIME
  else if (Utilities::match(keyword, w_TIME))
    parseTimeSetting();

  //... get control time of day if keyword == CLOCKTIME
  else if (Utilities::match(keyword, w_CLOCKTIME))
...
void ControlParser::parseLevelSetting(Network *network) {
  ...
  string keyword;
  sin >> keyword;
  if (Utilities::match(keyword, w_ABOVE))
    levelType = Control::HI_LEVEL;
  else if (Utilities::match(keyword, w_BELOW))
    levelType = Control::LOW_LEVEL;
  else
    throw InputError(InputError::INVALID_KEYWORD, keyword);
  ...
  if (node->type() == Node::TANK)
    controlType = Control::TANK_LEVEL;
  else
    controlType = Control::PRESSURE_LEVEL;
}
...
void ControlParser::createControl(Network *network) {
  ...
  control->setProperties(controlType, link, linkStatus, linkSetting, node,
                         nodeSetting, levelType, timeSetting);
}
