...
void ObjectParser::parseLine(string &line, int section) {
  ...
  case InputReader::PATTERN:
    ...
    if (network->indexOf(Element::PATTERN, s1) >= 0)
      break;
    ...
    if (Utilities::match(s2, w_Variable))
      type = Pattern::VARIABLE_PATTERN;
  ...
  case InputReader::CURVE:
    ...
    if (network->indexOf(Element::CURVE, s1) >= 0)
      break;
...
void PropertyParser::parseLine(string &line, int section) {
  ...
  // Link properties
  case InputReader::STATUS:
    parseLinkProperty(STATUS, id);
    break;
  case InputReader::LEAKAGE:
    parseLinkProperty(LEAKAGE, id);
    break;
  ...
  case InputReader::ENERGY:
    if (Utilities::match(id, w_Pump)) {
      ...
      optionParser.parseEnergyOption(network, tokens);
    break;
...
void PropertyParser::parseNodeProperty(int type, string &nodeName) {
  ...
  if (node == nullptr)
    throw InputError(InputError::UNDEFINED_OBJECT, nodeName);
  switch (type) {
  case InputReader::EMITTER:
...
void PropertyParser::parseLinkProperty(int type, string &linkName) {
  ...
  if (link == nullptr)
    throw InputError(InputError::UNDEFINED_OBJECT, linkName);
  switch (type) {
  case STATUS:
    linkParser.parseStatus(link, tokens);
    break;
  case LEAKAGE:
    linkParser.parseLeakage(link, tokens);
    break;
  case ENERGY:
    linkParser.parseEnergy(link, network, tokens);
    break;
  case BULK:
    linkParser.parseReaction(link, Link::BULK, tokens);
    break;
  case WALL:
    linkParser.parseReaction(link, Link::WALL, tokens);
    break;
  }
}
...
void PropertyParser::parseReactProperty(string &reactType) {
  ...
  if (Utilities::match(reactType, w_Bulk)) {
    ...
    optionParser.parseReactOption(network, tokens);
}
