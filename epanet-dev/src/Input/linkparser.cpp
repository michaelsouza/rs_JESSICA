...
static const char *valveTypeWords[] = {"PRV", "PSV", "FCV", "TCV",
                                       "PBV", "GPV", 0};
...
void LinkParser::parseLinkData(string &id, Network *nw,
                               vector<string> &tokens) {
  // ... read end nodes

  Link *link = nw->link(id);
  if (!link)
    throw InputError(InputError::UNDEFINED_OBJECT, id);
  if (tokens.size() < 4)
  ...
  switch (link->type()) {
  case Link::PIPE:
    parsePipeData(static_cast<Pipe *>(link), tokens);
    break;
  case Link::PUMP:
    parsePumpData(static_cast<Pump *>(link), nw, tokens);
    break;
  case Link::VALVE:
    parseValveData(static_cast<Valve *>(link), nw, tokens);
    break;
  default:
    throw InputError(InputError::UNDEFINED_OBJECT, id);
  }
...
void LinkParser::parseEnergy(Link *link, Network *network,
...
void LinkParser::parseReaction(Link *link, int type,
                               vector<string> &tokenList) {
  // Contents of tokenList are:
  // 0 - BULK/WALL keyword
  // 1 - link ID
...
void parseEndNodes(Link *link, Network *nw, vector<string> &tokenList) {
  ...
  if (link->fromNode == nullptr)
    throw InputError(InputError::UNDEFINED_OBJECT, tokens[1]);
...
void parsePipeData(Pipe *pipe, vector<string> &tokenList) {
  ...
  if (nTokens > 7 && tokens[7] != "*") {
    string s = tokens[7];
    if (Utilities::match(s, w_OPEN))
      pipe->initStatus = Link::LINK_OPEN;
    else if (Utilities::match(s, w_CLOSED))
      pipe->initStatus = Link::LINK_CLOSED;
    else if (Utilities::match(s, w_CV))
      pipe->hasCheckValve = true;
    else
      throw InputError(InputError::INVALID_KEYWORD, s);
  }
}
...
void parseValveData(Valve *valve, Network *network, vector<string> &tokenList) {
  ...
  if (vType < 0)
    throw InputError(InputError::INVALID_KEYWORD, tokens[4]);
  ...
  if (valve->valveType == Valve::GPV) {
    ...
    if (c < 0)
      throw InputError(InputError::UNDEFINED_OBJECT, tokens[5]);
    valve->initSetting = c;
  }
