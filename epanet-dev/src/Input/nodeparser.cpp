...
static void parseDemandData(Demand &demand, Network *nw,
                            vector<string> &tokens);
static void parseEmitterData(Network *nw, vector<string> &tokens, double &coeff,
                             double &expon, Pattern *pattern);

//-----------------------------------------------------------------------------

void NodeParser::parseNodeData(string &id, Network *nw,
                               vector<string> &tokens) {
  ...
  switch (node->type()) {
  case Node::JUNCTION:
    parseJuncData(static_cast<Junction *>(node), nw, tokens);
    break;
  case Node::RESERVOIR:
    parseResvData(static_cast<Reservoir *>(node), nw, tokens);
    break;
  case Node::TANK:
    parseTankData(static_cast<Tank *>(node), nw, tokens);
    break;
  default:
    throw InputError(InputError::UNDEFINED_OBJECT, id);
  }
...
void NodeParser::parseQualSource(Node *node, Network *nw,
...
void parseEmitterData(Network *nw, vector<string> &tokenList, double &coeff,
                      double &expon, Pattern *pattern) {
  // Contents of tokenList are:
  // 0 - junction ID
  // 1 - flow coefficient
