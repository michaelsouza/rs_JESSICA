...
static const char *stringOptionKeywords[] = {"HYDRAULICS_FILE",
                                             "",
...
static const char *reportOptionKeywords[] = {"SUMMARY", "ENERGY", "STATUS",
                                             "TRIALS", 0};

// ... Keywords for ValueOption enumeration in options.h
static const char *valueOptionKeywords[] = {"SPECIFIC_GRAVITY",
                                            "SPECIFIC_VISCOSITY",
                                            "DEMAND_MULTIPLIER",
                                            "MINIMUM_PRESSURE",
                                            "SERVICE_PRESSURE",
...
static const char *timeOptionKeywords[] = {"START_CLOCKTIME",
                                           "HYDRAULIC_TIMESTEP",
                                           "QUALITY_TIMESTEP",
                                           "PATTERN_TIMESTEP",
                                           "PATTERN_START",
...
// ... Deprecated keywords
static const char *deprecatedKeywords[] = {"SEGMENTS", "VERIFY",    "CHECKFREQ",
                                           "MAXCHECK", "DAMPLIMIT", "HTOL",
                                           "QTOL",     "RQTOL",     0};

// ... Single keywords (for EPANET2 compatibility)
static const char *epanet2Keywords[] = {
    "UNITS",   "PRESSURE",  "HEADLOSS",    "HYDRAULICS",
    "QUALITY", "VISCOSITY", "DIFFUSIVITY", "SPECIFIC",
    "TRIALS",  "ACCURACY",  "UNBALANCED",  "PATTERN",
    "DEMAND",  "EMITTER",   "TOLERANCE",   "MAP",
    0};

static const char *epanetQualKeywords[] = {"NONE", "AGE", "TRACE", "CHEMICAL",
                                           0};
...
void OptionParser::parseOption(Network *network, vector<string> &tokenList) {
  ...
  if (Utilities::findFullMatch(s1, deprecatedKeywords) >= 0)
    return;
  ...
  if (!value.empty())
    setOption(keyword, value, network);
}
...
void OptionParser::setOption(const string &keyword, const string &value,
  ...
  if (option >= 0) {
    int err = network->options.setOption((Options::StringOption)option, value);
    if (err)
      throw InputError(err, value);
    return;
  }
  ...
  if (option >= 0) {
    int err = network->options.setOption((Options::IndexOption)option, value,
                                         network);
    if (err)
      throw InputError(err, value);
    return;
  ...
  if (option < 0)
    throw InputError(InputError::INVALID_KEYWORD, keyword);
  double x;
...
void OptionParser::parseTimeOption(Network *network,
  ...
  if (s1 == w_DURATION)
    keyword = timeOptionKeywords[Options::TOTAL_DURATION];
  else if (s1 == w_STATISTIC)
    keyword = timeOptionKeywords[Options::REPORT_STATISTIC];
...
void OptionParser::parseEnergyOption(Network *network,
                                     vector<string> &tokenList) {
  // ... check for enough tokens
  ...
  if (Utilities::match(tokens[0], w_GLOBAL)) {
    ...
    // ... global pump efficiency
      ...
      throw InputError(InputError::INVALID_KEYWORD, tokens[1]);
  }
  ...
  // ... peak demand charge
    ...
    throw InputError(InputError::INVALID_KEYWORD, tokens[0]);
}

//-----------------------------------------------------------------------------

void OptionParser::parseReactOption(Network *network,
                                    vector<string> &tokenList) {
  // ... check for enough tokens

  if (tokenList.size() < 3)
    throw InputError(InputError::TOO_FEW_ITEMS, "");
  string *tokens = &tokenList[0];
...
void OptionParser::parseReportOption(Network *network,
                                     vector<string> &tokenList) {
  // ... check for no data
  ...
  if (Utilities::match(keyword, "PAGESIZE"))
    return;
  ...
  if (option >= 0) {
    ...
    if (option == Options::REPORT_STATUS &&
        Utilities::match(tokens[1], "FULL")) {
      network->options.setOption(Options::REPORT_TRIALS, true);
      network->options.setOption(Options::REPORT_STATUS, true);
      return;
    }

    if (Utilities::match(tokens[1], "YES"))
      value = true;
    else if (Utilities::match(tokens[1], "NO"))
      value = false;
    else
      throw InputError(InputError::INVALID_KEYWORD, tokens[1]);
  ...
  // ... parse report field options
    ...
    parseReportField(network, nTokens, tokens);
}
...
string OptionParser::getEpanet3Keyword(const string &s1, const string &s2,
  ...
  switch (i) {
  case 0:
    return indexOptionKeywords[Options::FLOW_UNITS];
  case 1:
    return indexOptionKeywords[Options::PRESSURE_UNITS];
  case 2:
    return stringOptionKeywords[Options::HEADLOSS_MODEL];
  case 3:
    return stringOptionKeywords[Options::HYD_FILE_NAME];
  case 4:
    break; // QUALITY keyword handled separately
  case 5:
    return valueOptionKeywords[Options::KIN_VISCOSITY];
  case 6:
    return valueOptionKeywords[Options::MOLEC_DIFFUSIVITY];
  case 7:
    return valueOptionKeywords[Options::SPEC_GRAVITY];
  case 8:
    return indexOptionKeywords[Options::MAX_TRIALS];
  case 9:
    return valueOptionKeywords[Options::RELATIVE_ACCURACY];
  case 10:
    value = s2; // make the 2nd token the option's value
    return indexOptionKeywords[Options::IF_UNBALANCED];
  case 11:
    return indexOptionKeywords[Options::DEMAND_PATTERN];
  case 12:
    return valueOptionKeywords[Options::DEMAND_MULTIPLIER];
  case 13:
    return valueOptionKeywords[Options::EMITTER_EXPONENT];
  case 14:
    return valueOptionKeywords[Options::QUAL_TOLERANCE];
  case 15:
    return stringOptionKeywords[Options::MAP_FILE_NAME];
  }
  return s1;
}
...
void OptionParser::parseQualOption(const string &s2, const string &s3,
  ...
  if (network->option(Options::QUAL_TYPE) == Options::TRACE) {
    if (s3.empty())
      throw InputError(InputError::UNDEFINED_OBJECT, s3);
  ...
  if (network->option(Options::QUAL_TYPE) == Options::CHEM && !s3.empty()) {
    ...
    if (s3U.compare("MG/L") == 0)
      ...
      throw InputError(InputError::INVALID_KEYWORD, s3);
  }
}
...
void OptionParser::parseReportItems(int nodeOrLink, Network *network,
  ...
  if (nodeOrLink == Element::LINK)
    option = Options::REPORT_LINKS;
  if (Utilities::match(tokens[1], "NONE"))
    value = Options::NONE;
  if (Utilities::match(tokens[1], "ALL"))
    value = Options::ALL;
...
void OptionParser::parseReportField(Network *network, int nTokens,
                                    string *tokens) {
  ...
  network->options.setReportFieldOption(type, index, enabled, precision,
