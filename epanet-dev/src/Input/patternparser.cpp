...
void PatternParser::parsePatternData(Pattern *pattern, vector<string> &tokens) {
  if (pattern->type == Pattern::FIXED_PATTERN) {
    ...
    parseVariablePattern(pattern, tokens);
}

//-----------------------------------------------------------------------------

void PatternParser::parseFixedPattern(Pattern *pattern,
  ...
  if (Utilities::match(tokens[1], "FIXED")) {
    ...
    if (nTokens > 2) {
      ...
      if (interval <= 0)
        throw InputError(InputError::INVALID_TIME, tokens[2]);
...
void PatternParser::parseVariablePattern(Pattern *pattern,
                                         vector<string> &tokenList) {
  // Formats are:
  ...
  if (Utilities::match(tokens[1], "VARIABLE"))
    return;
