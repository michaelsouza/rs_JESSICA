...
DemandModel *DemandModel::factory(const string model, const double expon_) {
  if (model == "FIXED")
    return new FixedDemandModel();
  else if (model == "CONSTRAINED")
    return new ConstrainedDemandModel();
  else if (model == "POWER")
    return new PowerDemandModel(expon_);
  else if (model == "LOGISTIC")
    return new LogisticDemandModel(expon_);
  else
    return nullptr;
}
...
double ConstrainedDemandModel::findDemand(Junction *junc, double p,
                                          double &dqdh) {
  dqdh = 0.0;
...
LogisticDemandModel::LogisticDemandModel(double expon_)
    : DemandModel(expon_), a(0.0), b(0.0) {}
