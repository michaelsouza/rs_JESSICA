...
HeadLossModel::HeadLossModel(double viscos) : viscosity(viscos) {}
...
HeadLossModel *HeadLossModel::factory(const string model, double viscos) {
  if (model == "H-W")
    return new HW_HeadLossModel(viscos);
  if (model == "D-W")
    return new DW_HeadLossModel(viscos);
  if (model == "C-M")
    return new CM_HeadLossModel(viscos);
  return nullptr;
}

// Head loss for a closed link

void HeadLossModel::findClosedHeadLoss(double flow, double &headLoss,
                                       double &gradient) {
...
void HeadLossModel::addCVHeadLoss(double q, double &headLoss,
                                  double &gradient) {
...
void HW_HeadLossModel::setResistance(Pipe *pipe) {
  pipe->resistance = 4.727 * pipe->length / pow(pipe->roughness, HW_EXP) /
...
void CM_HeadLossModel::findHeadLoss(Pipe *pipe, double flow, double &headLoss,
                                    double &gradient) {
  double q = abs(flow);
  double r = pipe->resistance;
  double k = pipe->lossFactor;
