...
LeakageModel *LeakageModel::factory(const string model, const double ucfLength_,
                                    const double ucfFlow_) {
  if (model == "POWER")
    return new PowerLeakageModel(ucfLength_, ucfFlow_);
  else if (model == "FAVAD")
    return new FavadLeakageModel(ucfLength_);
  else
    return nullptr;
}
...
PowerLeakageModel::PowerLeakageModel(const double ucfLength_,
                                     const double ucfFlow_) {
  lengthUcf = ucfLength_;
  flowUcf = ucfFlow_;

  if (lengthUcf == 1.0)
    pressureUcf = PSIperFT;
  else
    pressureUcf = MperFT;
}
