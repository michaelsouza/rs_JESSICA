...
//  Update a chemical's pipe concentration after reaction over a given time
//  step.
...
double ChemModel::pipeReact(Pipe *pipe, double c, double tstep) {
  ...
  if (kb != 0.0)
    dCdT = findBulkRate(kb, pipeOrder, c) * pipeUcf;

  double kw = pipe->wallCoeff / SECperDAY * wallUcf;
  if (kw != 0.0)
    dCdT += findWallRate(kw, pipe->diameter, wallOrder, c);
...
//  Update a chemical's tank concentration after reaction over a given time
//  step.
...
double ChemModel::findBulkRate(double kb, double order, double c) {
  ...
  // ... n-th order kinetics:
    ...
    if (cLimit == 0.0)
      ...
      c1 = max(0.0, Utilities::sign(kb) * (cLimit - c));
