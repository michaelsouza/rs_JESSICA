...
const char *TankMixModel::MixingModelWords[] = {"MIXED", "2COMP", "FIFO",
                                                "LIFO", 0};

//-----------------------------------------------------------------------------

TankMixModel::TankMixModel()
    : type(MIX1), cTol(0.0), fracMixed(0.0), cTank(0.0), vMixed(0.0),
      firstSeg(nullptr), lastSeg(nullptr) {}
...
double TankMixModel::findQuality(double vNet, double vIn, double wIn,
                                 SegPool *segPool) {
  switch (type) {
  case MIX2:
    return findMIX2Quality(vNet, vIn, wIn);
  case FIFO:
    return findFIFOQuality(vNet, vIn, wIn, segPool);
  case LIFO:
    return findLIFOQuality(vNet, vIn, wIn, segPool);
  default:
    return findMIX1Quality(vNet, vIn, wIn);
  }
}
...
double TankMixModel::findMIX2Quality(double vNet, double vIn, double wIn) {
  ...
  if (vNet < 0.0) {
    ...
    if (vIn + vTransfer > 0.0) {
      mixZone->c = (mixZone->c * mixZone->v + wIn + stagZone->c * vTransfer) /
  ...
  if (vTransfer > 0.0) {
    ...
    if (vNet > 0.0)
      ...
      stagZone->v = max(0.0, stagZone->v - vTransfer);
  } else {
    mixZone->v += vNet;
...
double TankMixModel::findFIFOQuality(double vNet, double vIn, double wIn,
  ...
  if (vIn > 0.0) {
    ...
    double cIn = wIn / vIn;
    if (lastSeg && abs(lastSeg->c - cIn) < cTol)
      lastSeg->v += vIn;
...
double TankMixModel::findLIFOQuality(double vNet, double vIn, double wIn,
  ...
  if (vNet > 0.0) {
    ...
    double cIn = wIn / vIn;
    if (firstSeg && abs(firstSeg->c - cIn) < cTol)
      firstSeg->v += vNet;
