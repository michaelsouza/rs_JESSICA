...
int ProjectWriter::writeFile(const char *fname, Network *nw) {
  ...
  if (!fout.is_open())
    return FileError::CANNOT_OPEN_INPUT_FILE;
  ...
  for (Link *link : network->links) {
    if (link->type() == Link::PIPE) {
      ...
      if (pipe->hasCheckValve)
        fout << "CV\n";
      else if (link->initStatus == Link::LINK_CLOSED)
        fout << "CLOSED\n";
      else
        fout << "\n";
    }
  ...
  for (Link *link : network->links) {
    if (link->type() == Link::PUMP) {
      ...
      if (pump->pumpCurve.horsepower > 0.0) {
        fout << setw(8) << "POWER";
        fout << setw(12)
             << pump->pumpCurve.horsepower * network->ucf(Units::POWER);
      }
  ...
  for (Node *node : network->nodes) {
    if (node->type() == Node::JUNCTION) {
      ...
      if (emitter) {
        ...
        fout << setw(12) << fixed << setprecision(4);
        fout << emitter->expon;
        if (emitter->timePattern != 0)
          fout << setw(16) << emitter->timePattern->name;
        fout << "\n";
      }
    }
  }
...
void ProjectWriter::writeStatus() {
  ...
  for (Link *link : network->links) {
    if (link->type() == Link::PUMP) {
      ...
      if (link->initStatus == Link::LINK_OPEN ||
          link->initStatus == Link::LINK_CLOSED) {
        fout << left << setw(16) << link->name << " ";
        if (link->initStatus == Link::LINK_OPEN)
          fout << "OPEN\n";
        else
  ...
  for (Pattern *pattern : network->patterns) {
    if (pattern->type == Pattern::FIXED_PATTERN) {
      ...
      if (pattern->timeInterval() > 0)
        fout << Utilities::getTime(pattern->timeInterval());
      int k = 0;
      int i = 0;
      int n = pattern->size();
      while (i < n) {
