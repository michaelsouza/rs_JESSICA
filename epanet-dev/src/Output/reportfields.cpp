...
const char *ReportFields::NodeFieldNames[] = {
    ...
    "Deficit",   "Outflow", "Quality",  0};

const char *ReportFields::LinkFieldNames[] = {
    "Length",    "Diameter", "Flow Rate", "Leakage", "Velocity",
    "Head Loss", "Status",   "Setting",   "Quality", 0};

ReportFields::ReportFields() { setDefaults(); }
...
void ReportFields::setField(int type, int index, int enabled, int precision,
                            double lowerLimit, double upperLimit) {
  Field *field;
  if (type == Element::NODE) {
    if (index < 0 || index >= NUM_NODE_FIELDS)
      return;
    field = &nodeFields[index];
  } else if (type == Element::LINK) {
    if (index < 0 || index >= NUM_LINK_FIELDS)
      return;
