...
int mmdelm_(int *mdnode, int *xadj, int *adjncy, int *dhead, int *dforw,
            int *dbakw, int *qsize, int *llist, int *marker, int *maxint,
            int *tag);
...
int sp_genmmd(int *neqns, int *xadj, int *adjncy, int *invp, int *perm,
  ...
  /*        ------------------------------------------------ */
  *nofsub = 0;
  mmdint_(neqns, &xadj[1], &adjncy[1], &dhead[1], &invp[1], &perm[1], &qsize[1],
          &llist[1], &marker[1]);
...
void sp_smbfct(int neqns, int *xadj, int *adjncy, int *perm, int *invp,
               int *xlnz, int &maxlnz, int *xnzsub, int *nzsub, int &maxsub,
               int *rchlnk, int *mrglnk, int *marker, int &flag) {
  /* Local variables */
  ...
  /* FOR EACH COLUMN KNZ COUNTS THE NUMBER OF NONZEROS IN COLUMN K ACCUMULATED
   * IN RCHLNK. */
  np1 = neqns + 1;
  for (k = 1; k <= neqns; ++k) {
    knz = 0;
  ...
  L1000:
    ...
    /* COPY THE STRUCTURE OF L(*,K) FROM RCHLNK TO THE DATA STRUCTURE (XNZSUB,
     * NZSUB) */
  L1200:
    nzbeg = nzend + 1;
    nzend = nzend + knz;
...
*
*******************************************************************/
void sp_solve(int neqns, int *xlnz, double *lnz, int *xnzsub, int *nzsub,
              double *diag, double *rhs) {
  int i, isub;
  double rhsj, s;
