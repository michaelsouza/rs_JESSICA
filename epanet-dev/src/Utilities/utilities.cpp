...
const std::string &TempFile::getFileName() const { return fileName; }
...
bool Utilities::getTmpFileName(string &fname) {
  ...
  if (rtnValue == 0 || rtnValue > MAX_PATH)
    return false;
...
int Utilities::getSeconds(const string &strTime, const string &strUnits) {
  ...
  if (strTime.find(':', 0) != string::npos) {
    int h = 0, m = 0, s = 0;
    if (sscanf(strTime.c_str(), "%d:%d:%d", &h, &m, &s) == 0)
      return -1;
  ...
  if (match(strUnits, s_Day))
    return (int)(3600. * 24. * t);
  if (match(strUnits, s_Hour))
    return (int)(3600. * t);
  if (match(strUnits, s_Minute))
    return (int)(60. * t);
  if (match(strUnits, s_Second))
    return (int)t;
